...
DESCRIPTION : This code read a PMsensor and publish the data to a server.
  ...
  and pre-chamber to acconditionate the samples. This prechamber count with
    - heater  
    - a Termo sensor PT100  to read the temperature of the pre chamber, and  
...
- make id programmable
- probe 2 dht
- probe RTC
- make RTC programmable
- probe datalogger SD
- add RGB LED feature
...
//DHT sensor
DHT dht_node(DHT_NODE_SENSOR_PIN,DHTTYPE);
DHT dht_ext(DHT_EXT_SENSOR_PIN,DHTTYPE);
...
{ 
  ...
  t_ext_avg  = t_ext_sum / N_SAMPLES;
  h_ext_avg  = h_ext_sum / N_SAMPLES;
...
void writeDatalogger()
{
    dataMsg = String(lastTimeUnix)+","+String(data.pm25_standard)+","+String(t_node)+String(h_node)+","+String(t_pms)+String(h_pms)+ "\r\n";
...
void readSensors()
{
  ...
  //h_ext  = sht31.readHumidity();
  //t_ext  = sht31.readTemperature();
