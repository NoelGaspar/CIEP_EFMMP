...
const char* json_rx =  "{\"id\":\"em-ciep-01\",\"cmd\":\"LED\",\"arg1\":48.756,\"arg2\":48.75608}";
...
//LED
bool led_state  = false; // for status LED
...
void readSensors()
{
  ...
  DHT_rh_buff[pms_idx]    = dht.readTemperature();
  DHT_temp_buff[pms_idx]  = dht.readHumidity();
  ...
  dht_temp_avg  += DHT_temp_buff[pms_idx];
    ...
    PMS_rh_buff[pms_idx]    = data.pm_t;
    PMS_temp_buff[pms_idx]  = data.pm_h;
    dht_rh_avg    += DHT_rh_buff[pms_idx];
    dht_temp_avg  += DHT_temp_buff[pms_idx];
  #endif
