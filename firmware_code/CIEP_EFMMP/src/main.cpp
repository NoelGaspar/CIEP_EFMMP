...
#define STATUS_LED_PIN  15  //STATUS EXTERNAL LED

//#define SDA_PIN   18  //I2C SDA
//#define SCK_PIN   19  //I2C SCL

#define SCL_PIN   34  // SPI SCL
#define MOSI_PIN  32  // SPI MOSI
#define CS_PIN    33  // SPI CS

//#define PWM1_PIN  13  //HEATER
//#define PWM2_PIN  15  //COOLER
...
#define TIMER_DEFAULT_TS  4           // TIMER DEFAULT PERIOD. IN SECONDS 
...
const char apn[]      = "freeeway"; // APN from PRIMCARDS
...
-----------------*/
//JSON to SEND
//const size_t capacity_tx = JSON_ARRAY_SIZE(100) + JSON_OBJECT_SIZE(7);
...
const char* json_rx =  "{\"id\":\"em-ciep-05\",\"cmd\":\"LED\",\"arg1\":48.756,\"arg2\":48.75608}";
...
//PMS read port
SoftwareSerial SerialPMS(PMS_RX,PMS_TX);
...
//  float  rh = 0;
//};
//struct SHT31_data ext_sens_data; // 
...
void setup()
{
  ...
  //setup Timer
  timer = timerBegin(3, 8000, true);    //timer 4, prescaler 8000, counting up
...
void publishSensor(const char * resp)
{ 
  const size_t capacity_tx = JSON_ARRAY_SIZE(20) + JSON_OBJECT_SIZE(7);
  DynamicJsonDocument doc_tx(capacity_tx);
  ...
  for( uint8_t i = 0; i < MAX_PMS_BUFF_SIZE ;i++)
  {
    sht_rh_avg    += SHT_rh_buff[i];
    sht_temp_avg  += SHT_temp_buff[i];
    dht_rh_avg    += DHT_rh_buff[i];
    dht_temp_avg  += DHT_temp_buff[i];
...
void readSensors()
{
  readPMSdata(&SerialPMS);
  PMS_buff[pms_idx]       = data.pm25_standard;
   
  SHT_rh_buff[pms_idx]    = sht31.readTemperature();
  SHT_temp_buff[pms_idx]  = sht31.readHumidity();
