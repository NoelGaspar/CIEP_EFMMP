...
//NODE DEFINITIONS
#define SENSOR_ID         "CIEP-11"   // NODE ID//"UDD-01"
#define VESRION           "V1.1.0"    // ACTUAL CODE VERSION
...
#define N_SAMPLES         80          // NUMBER OF SAMPLES TO CONSIDERAR IN THE AVERAGE OF SAMPLES
...
#define MQTT_PUBLISH_CH   "CIEP/tx"        // CHANNEL TO PUBLISH
#define MQTT_RECEIVER_CH  "CIEP/rx"        // CHANNEL TO SUBSCRIBE (RECIVE COMMANDS)


/*
#define MQTT_PUBLISH_CH   "UDD/tx"        // CHANNELS FOR UDD
#define MQTT_RECEIVER_CH  "UDD/rx"        // CHANNELS FOR UDD
*/
...
//HEATER 
uint8_t heater_value = 0;     // VARIABLE TO STORE THE HEATER OUTPUT VALUE. RANGE : 0 TO 255
...
void setup()
{
  ...
  if(!file)
  {
    ...
    writeFile(SD, "/data.txt", "time, pm25, ts,hs,tn,hn \r\n");
  }
  else
  {
...
void writeDatalogger()
{
    dataMsg = String(lastTimeUnix)+","+String(data.pm25_standard)+","+String(data.pm_t)+","+String(data.pm_h)+","+String(t_node)+","+String(h_node)+"\r\n";
